---
name: pi-postman
description: "Send structured handoffs between agent sessions on the same machine. Use when the user asks to 'hand off', 'send to the other tab', 'postman', 'message the author session', or wants to transfer distilled context (e.g. a code review) to another open Pi session."
---
...
- Sharing full session context (out of scope; pi-postman is for distilled handoffs)
...
A good postman message is **distilled, not dumped**. The recipient is a fresh agent context — it does not have your conversation, your file reads, or your reasoning chain. Include only:
...
**Always** preview the full outbound message and ask the user to approve before calling `postman_send`. Format:
...
When the user asks to "check the postman inbox", "see new messages", or similar:

1. Call `postman_inbox()` first. Show the list with id, sender, subject, kind, priority.
2. For each message the user wants to read, call `postman_read(id)`. Show the body.
3. If the message references a file path or PR, follow it.
4. If the message is a `question`, `blocker`, or `decision`, ask the user how they want to respond and use `postman_reply(id, kind, body)`.
...
You:
1. Discover sessions with `postman_sessions()`. Identify the author session (likely named something like `pi-author` or matching the worktree).
...
You:
1. `postman_sessions()` to find the recipient.
...
User in tab B: *"Anything in the postman inbox?"*

You:
1. `postman_inbox(limit: 10)`.
2. If empty, say so and stop.
3. If non-empty, list with sender, kind, subject. Ask which to read.
4. For chosen messages, `postman_read(id)`, show the body, follow any file references.

## Rules

1. **Approve every outbound message.** No exceptions.
2. **Distill, don't dump.** A postman message that includes the entire conversation defeats the point.
...
7. **Threads are conversations.** If you're replying to a previous message, use `postman_reply` (which preserves threading), not a fresh `postman_send`.
